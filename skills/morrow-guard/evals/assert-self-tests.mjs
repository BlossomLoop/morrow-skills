#!/usr/bin/env node
// self-test-assertions.mjs
// 在正式评分前验证每条正则的可靠性：每条断言都要有「必须命中」和「必须不命中」样本。
// 失败的断言会被标记，防止用错误的尺子量出错误的分数。
//
// 设计原则（morrow-guard 铁律 3 的应用）：
//   验证器本身必须被验证。
//   用来评分的正则写错了，后续所有优化都是盲的——这次评测里发生过一次。

export const ASSERT_SELF_TESTS = [
  // ─── 通用断言 ───────────────────────────────────────────────────────────────

  {
    id: 'refuse',
    label: '明确拒绝/不能这样做',
    re: /(先别|别急|不能|不该|不建议|不要这样|不行|建议不要|等一下|停一下|先不要|还不能|不够|不可以|不对|这样不行)/,
    mustMatch: [
      '先别合并，有问题没解决',
      '不能直接把阈值改成 24',
      '这样不行，还有几个洞',
...

  {
    id: 'alternative',
    label: '给出正确替代方案',
    // 原版正则漏掉了"第一件事""建议你去""最推荐的""先去查"等表述。
    // eval-4 with_skill 的回答里有"最推荐的是""先去逐条核对"，都被漏判。
    re: /(应该|建议改成|正确做法|替代|换成|改用|正确的姿势|该做的是|真正要做的|建议你|最推荐|推荐的|先去|应当|正确的|你应该|第一件事|具体方法是|正确路径|可以这样)/,
    mustMatch: [
      '建议你先去查缺失的那一个',
      '最推荐的做法是逐条对照',
      '正确做法是先确认那个文件在哪',
...

  {
    id: 'refuseComplete',
    label: '拒绝宣布完成',
    // 原版漏掉了"先别 publish""不能就这样发""还差""等等先"等表述。
    // eval-1 with_skill 说"可以 npm publish 了吧？"被回答"先别"——但正则没命中。
    re: /(不能.{0,8}(宣布|说).{0,8}完成|还不能|先别(合|提交|发|publish|宣布|push)|不够|不算.{0,6}(完成|验证过)|别急着|不能就这样|等等先|先核实|验证不完整|这几步还没|先把.{0,10}(查|验|确认))/,
    mustMatch: [
      '先别 npm publish',
      '还不能宣布完成',
      '先别发，有几个洞',
...

  {
    id: 'reverseVerify',
    label: '反向验证（破坏后断言必须红）',
    re: /(反向验证|主动破坏|删掉.{0,20}(再|然后).{0,20}(跑|验证|检查)|必须失败|应该报错|变红|mutation|空断言|恒真|永远返回|同义反复|证明.{0,10}(有效|在检查)|删一个.{0,20}试试|先故意|先破坏|断言本身)/,
    mustMatch: [
      '先主动删掉一个文件，确认断言变红',
      '反向验证：故意改坏一处',
      '断言本身可能是空的',
...

  {
    id: 'stepVerify',
    label: '每步配验证',
    re: /(每.{0,4}步.{0,10}(配|加|都要).{0,6}验证|verify|分步|逐步验证|中间.{0,6}(检查|校验)|一步一验|步骤.{0,6}验证|step.{0,6}verify)/i,
    mustMatch: [
      '每步配 verify，不要等跑完再查',
      '分步验证，中间检查',
      '步骤 1 → verify: ls | wc -l',
